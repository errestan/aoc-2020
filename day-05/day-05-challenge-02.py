...
if __name__ == "__main__":
    ...
    seats = sorted([decode_seat(line.strip()) for line in input])
    empty = [seat for seat in range(seats[0], seats[-1] + 1) if seat not in seats]

    print(f"Empty seat(s): {', '.join([str(i) for i in empty])}")
